...
import com.frierenflight.entity.ExplosionChantForgeEntity;
...
public class ExplosionChantForgeRenderer extends EntityRenderer<ExplosionChantForgeEntity> {

    public ExplosionChantForgeRenderer(EntityRendererProvider.Context context) {
        super(context);
    }

    @Override
    public ResourceLocation getTextureLocation(ExplosionChantForgeEntity entity) {
        return ExplosionVFXRenderer.TEX_MAGIC_CIRCLE;
    }

    @Override
    public boolean shouldRender(ExplosionChantForgeEntity entity, Frustum frustum, double camX, double camY, double camZ) {
        return true; // Never cull chanting circle at player's feet
    }

    @Override
    public void render(ExplosionChantForgeEntity entity, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight) {
