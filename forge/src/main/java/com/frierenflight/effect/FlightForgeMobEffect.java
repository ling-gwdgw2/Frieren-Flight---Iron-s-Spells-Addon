...
public class FlightForgeMobEffect extends MagicMobEffect {
    public FlightForgeMobEffect() {
            ...
            if (player.getAbilities().flying) {
                if (player.level().isClientSide) {
                    ...
                    if (!player.isCreative() && player.tickCount % 20 == 0) {
                        int drain = FrierenFlightCommon.getManaDrain(amplifier);
                    ...
                    if (player.getServer() != null) {
                        player.getServer().execute(() -> {
                            if (player.isAlive() && !player.isDeadOrDying()) {
                                player.addEffect(new MobEffectInstance(MobEffects.SLOW_FALLING, FrierenFlightCommon.SLOW_FALLING_TICKS, 0, false, false, true));
                            }
                        });
                    } else {
                        player.addEffect(new MobEffectInstance(MobEffects.SLOW_FALLING, FrierenFlightCommon.SLOW_FALLING_TICKS, 0, false, false, true));
