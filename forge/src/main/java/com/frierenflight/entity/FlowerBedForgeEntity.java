...
public class FlowerBedForgeEntity extends Entity implements IEntityAdditionalSpawnData {
    ...
    public FlowerBedForgeEntity(EntityType<? extends FlowerBedForgeEntity> entityType, Level level) {
        super(entityType, level);
        this.noPhysics = true;
    }

    public FlowerBedForgeEntity(Level level, double x, double y, double z, float maxRadius) {
        this(level, x, y, z, maxRadius, 1200, 1);
    }

    public FlowerBedForgeEntity(Level level, double x, double y, double z, float maxRadius, int maxAge) {
        this(level, x, y, z, maxRadius, maxAge, 1);
    }

    public FlowerBedForgeEntity(Level level, double x, double y, double z, float maxRadius, int maxAge, int spellLevel) {
        this(ModForgeEntities.FLOWER_BED.get(), level);
        setPos(x, y, z);
        ...
        if (!level().isClientSide && level() instanceof ServerLevel serverLevel) {
            ...
            if (tickCount % 20 == 0) {
                ...
                for (Player playerTarget : players) {
                    playerTarget.addEffect(new MobEffectInstance(ModForgeEffects.FLOWER_BLESSING.get(), 60, amplifier, false, true, true));
                }
