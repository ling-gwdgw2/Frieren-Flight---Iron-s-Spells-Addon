...
public class ExplosionCataclysmForgeEntity extends Entity implements IEntityAdditionalSpawnData {
    ...
    public ExplosionCataclysmForgeEntity(EntityType<? extends ExplosionCataclysmForgeEntity> entityType, Level level) {
        super(entityType, level);
        this.noPhysics = true;
    }

    public ExplosionCataclysmForgeEntity(Level level, LivingEntity owner, float damage, float blastRadius) {
        this(ModForgeEntities.EXPLOSION_CATACLYSM.get(), level);
        this.owner = owner;
        if (owner != null) {
    ...
    public void tick() {
        ...
        if (!level().isClientSide && level() instanceof ServerLevel serverLevel) {
            ...
            if (tickCount == 1) {
                serverLevel.playSound(null, getX(), getY(), getZ(), com.frierenflight.registry.ModForgeSounds.MEGUMIN_CHARGING.get(), SoundSource.PLAYERS, 10.0f, 1.0f);
            }

            // Phase 2: Stratosphere Plasma Beam Descent (t = 65)
            if (tickCount == 65) {
                serverLevel.playSound(null, getX(), getY(), getZ(), com.frierenflight.registry.ModForgeSounds.MEGUMIN_RAY.get(), SoundSource.PLAYERS, 18.0f, 1.0f);
            }
            ...
            if (tickCount == 100) {
                ...
                serverLevel.playSound(null, getX(), getY(), getZ(), com.frierenflight.registry.ModForgeSounds.MEGUMIN_BLAST.get(), SoundSource.PLAYERS, 25.0f, 1.0f);
                ...
                for (LivingEntity victim : targets) {
                    ...
                    if (dist <= blastRadius * blastRadius) {
                        ...
                        if (owner != null && ModForgeSpells.EXPLOSION != null && ModForgeSpells.EXPLOSION.get() != null) {
                            DamageSources.applyDamage(victim, entityDmg, ModForgeSpells.EXPLOSION.get().getDamageSource(owner));
                        } else {
