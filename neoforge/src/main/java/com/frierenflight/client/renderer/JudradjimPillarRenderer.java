...
public class JudradjimPillarRenderer extends EntityRenderer<JudradjimPillarEntity> {
    private static final int FRAME_COUNT = 6;
    private static final ResourceLocation[] FRAMES = new ResourceLocation[FRAME_COUNT];
    private static final ResourceLocation SHOCKWAVE_TEXTURE = ResourceLocation.fromNamespaceAndPath(FrierenFlightCommon.MODID, "textures/entity/judradjim_shockwave.png");

    static {
        for (int i = 0; i < FRAME_COUNT; i++) {
            FRAMES[i] = ResourceLocation.fromNamespaceAndPath(FrierenFlightCommon.MODID, "textures/entity/judradjim_pillar_" + i + ".png");
        }
    }

    private static final float HEIGHT = 38.0f;

    public JudradjimPillarRenderer(EntityRendererProvider.Context context) {
        super(context);
    }

    @Override
    public ResourceLocation getTextureLocation(JudradjimPillarEntity entity) {
        int frame = Math.abs(entity.tickCount) % FRAME_COUNT;
        return FRAMES[frame];
    }

    @Override
    public boolean shouldRender(JudradjimPillarEntity entity, Frustum frustum, double camX, double camY, double camZ) {
        return true; // Never culled by camera frustum with shaders
    }

    @Override
    public void render(JudradjimPillarEntity entity, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight) {
    ...
    private void renderCrossPlanes(PoseStack poseStack, VertexConsumer builder, float width, float height, int alpha, float vOffset) {
        ...
        for (int plane = 0; plane < 3; plane++) {
            ...
            // Front Face
            addVertex(builder, pMat, pPose, -halfW, 0, 0, 0.0f, v2, 255, 255, 255, alpha, 0, 0, 1);
            addVertex(builder, pMat, pPose, halfW, 0, 0, 1.0f, v2, 255, 255, 255, alpha, 0, 0, 1);
            addVertex(builder, pMat, pPose, halfW, height, 0, 1.0f, v1, 255, 255, 255, alpha, 0, 0, 1);
            addVertex(builder, pMat, pPose, -halfW, height, 0, 0.0f, v1, 255, 255, 255, alpha, 0, 0, 1);
    ...
    private void renderCylinder(PoseStack poseStack, VertexConsumer builder, float radius, float height, int alpha, float vOffset, int sides) {
        ...
        for (int i = 0; i < sides; i++) {
            ...
            // Outside Face
            addVertex(builder, mat, pose, x1, 0, z1, u1, v2, 255, 255, 255, alpha, 0, 1, 0);
            addVertex(builder, mat, pose, x2, 0, z2, u2, v2, 255, 255, 255, alpha, 0, 1, 0);
            addVertex(builder, mat, pose, x2, height, z2, u2, v1, 255, 255, 255, alpha, 0, 1, 0);
            addVertex(builder, mat, pose, x1, height, z1, u1, v1, 255, 255, 255, alpha, 0, 1, 0);
    ...
    private void renderShockwave(PoseStack poseStack, VertexConsumer builder, float radius, int alpha, float rotation) {
        ...
        // Top Face
        addVertex(builder, mat, pose, -radius, 0, -radius, 0.0f, 0.0f, 255, 255, 255, alpha, 0, 1, 0);
        addVertex(builder, mat, pose, -radius, 0, radius, 0.0f, 1.0f, 255, 255, 255, alpha, 0, 1, 0);
        addVertex(builder, mat, pose, radius, 0, radius, 1.0f, 1.0f, 255, 255, 255, alpha, 0, 1, 0);
        addVertex(builder, mat, pose, radius, 0, -radius, 1.0f, 0.0f, 255, 255, 255, alpha, 0, 1, 0);
