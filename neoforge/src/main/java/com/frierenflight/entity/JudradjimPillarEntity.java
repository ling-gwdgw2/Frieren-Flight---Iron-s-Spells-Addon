...
public class JudradjimPillarEntity extends Entity {
    ...
    public JudradjimPillarEntity(EntityType<? extends JudradjimPillarEntity> entityType, Level level) {
        super(entityType, level);
        this.noPhysics = true;
    }

    public JudradjimPillarEntity(Level level, LivingEntity owner, float damage, float blastRadius) {
        this(ModEntities.JUDRADJIM_PILLAR.get(), level);
        this.owner = owner;
        if (owner != null) {
    ...
    public void tick() {
        ...
        if (tickCount == 1) {
            if (!level().isClientSide && level() instanceof ServerLevel serverLevel) {
                ...
                serverLevel.playSound(null, getX(), getY(), getZ(), SoundEvents.GENERIC_EXPLODE.value(), SoundSource.PLAYERS, 2.5f, 0.85f);
                ...
                for (LivingEntity victim : targets) {
                    ...
                    if (dist <= blastRadius * blastRadius) {
                        ...
                        if (owner != null) {
                            DamageSources.applyDamage(victim, entityDamage, ModSpells.JUDRADJIM.get().getDamageSource(owner));
                        } else {
