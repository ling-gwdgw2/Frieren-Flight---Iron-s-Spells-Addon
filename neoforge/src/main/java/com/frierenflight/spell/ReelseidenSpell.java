...
public class ReelseidenSpell extends AbstractSpell {
    private final ResourceLocation spellId = ResourceLocation.fromNamespaceAndPath(FrierenFlightCommon.MODID, "reelseiden");
    ...
    public void onCast(Level level, int spellLevel, LivingEntity entity, CastSource castSource, MagicData playerMagicData) {
        if (!level.isClientSide && level instanceof ServerLevel serverLevel) {
            ...
            serverLevel.playSound(null, entity.getX(), entity.getY(), entity.getZ(), SoundEvents.TRIDENT_THROW.value(), SoundSource.PLAYERS, 1.2f, 2.0f);

            // 2. Spawn Custom 3D Crescent Blade Entity
            ReelseidenSlashEntity slash = new ReelseidenSlashEntity(serverLevel, entity, damage);
